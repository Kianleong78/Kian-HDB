...
export const PropertyAnalysisView: React.FC<PropertyAnalysisViewProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 text-slate-800 dark:text-slate-100 transition-colors">
      {/* Header Banner */}
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-3 py-1 rounded-full text-xs font-black uppercase bg-emerald-100 dark:bg-emerald-500/20 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-500/40">
                {property.verdict}
              </span>
              ...
              <span className="px-3 py-1 rounded-full text-xs font-semibold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                Lease: {property.leaseCommenceDate} ({property.remainingLease} yrs left)
              </span>
          ...
          {/* Large Hero AI Score Badge & CTAs */}
          <div className="flex flex-col sm:flex-row lg:flex-col items-start lg:items-end gap-3 shrink-0">
            <div className="bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-700 p-4 rounded-2xl flex items-center gap-4 shadow-sm">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-emerald-600 to-teal-400 text-white flex items-center justify-center font-black text-2xl shadow-md">
                {property.aiMarketScore}
              </div>
              <div>
                <div className="text-[10px] uppercase font-bold text-emerald-600 dark:text-emerald-400 tracking-wider">AI Market Score</div>
                <div className="text-sm font-bold text-slate-900 dark:text-white">Out of 100</div>
                <div className="text-[11px] text-slate-500 dark:text-slate-400">{property.aiConfidenceScore}% AI Confidence</div>
              </div>
            </div>

            <div className="flex items-center gap-2 w-full">
              <button
                id="analysis-methodology-btn"
                onClick={() => setActiveTab('methodology')}
                className="px-3.5 py-3 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold transition flex items-center gap-1.5 border border-slate-200 dark:border-slate-700"
                title="Understand how this AI valuation was calculated"
              >
              ...
              <button
                ...
                className="flex-1 px-5 py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs transition-all shadow-md flex items-center justify-center gap-1.5"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm">
          <div className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Asking Price</div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-emerald-300 dark:border-emerald-800/80 p-5 rounded-2xl shadow-sm">
          ...
          <div className="mt-2 text-xs text-slate-500 dark:text-slate-400">
            Range: S${(property.fairPriceMin / 1000).toFixed(0)}k - S${(property.fairPriceMax / 1000).toFixed(0)}k
          </div>
        </div>

        {/* Price PSF & Capital Growth */}
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm">
          <div className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Unit Price PSF</div>
          <div className="text-2xl font-black text-slate-900 dark:text-white font-mono mt-1">
            S${property.pricePsf} <span className="text-xs text-slate-400 font-normal">/ sqft</span>
          </div>
          <div className="mt-2 text-xs text-emerald-600 dark:text-emerald-400 font-medium">
            5-Yr Forecast: +{property.projected5YrGrowthPct}%
          </div>
        </div>

        {/* Remaining Lease & Bala's Retention */}
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm">
          <div className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Remaining Lease</div>
          <div className="text-2xl font-black text-sky-600 dark:text-sky-400 font-mono mt-1">
            {property.remainingLease} Years
          </div>
          <div className="mt-2 text-xs text-slate-600 dark:text-slate-300">
            SLA Retention: <span className="font-bold text-slate-900 dark:text-white">{property.balasCurveRetentionPct}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
          <div>
            <div className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Decision Playbook</div>
            <h3 className="text-xl font-bold text-slate-900 dark:text-white mt-0.5">Tactical Negotiation & Suggested Pricing</h3>
          </div>
          ...
          <div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-xl flex items-center border border-slate-200 dark:border-slate-700 text-xs">
            <button
              ...
              className={`px-4 py-1.5 rounded-lg font-bold transition-all flex items-center gap-1.5 ${
                activeNegotiationTab === 'buyer' ? 'bg-emerald-500 text-slate-950 shadow' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-lg font-bold transition-all flex items-center gap-1.5 ${
                activeNegotiationTab === 'seller' ? 'bg-emerald-500 text-slate-950 shadow' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
        ...
        {activeNegotiationTab === 'buyer' ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl">
              <div className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Aggressive Opening Offer</div>
            ...
            <div className="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-500/40 p-5 rounded-2xl">
            ...
            <div className="bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl">
              <div className="text-xs font-bold text-amber-600 dark:text-amber-400 uppercase tracking-wider">Walk-Away Ceiling Price</div>
              <div className="text-2xl font-black text-slate-900 dark:text-white font-mono mt-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl">
              <div className="text-xs font-bold text-sky-600 dark:text-sky-400 uppercase tracking-wider">Fast-Liquidity Listing</div>
              <div className="text-2xl font-black text-slate-900 dark:text-white font-mono mt-2">
            ...
            <div className="bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl">
              <div className="text-xs font-bold text-purple-600 dark:text-purple-400 uppercase tracking-wider">Aggressive Premium Listing</div>
              <div className="text-2xl font-black text-slate-900 dark:text-white font-mono mt-2">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Cluster Benchmark Comps</div>
            <h3 className="text-xl font-bold text-slate-900 dark:text-white mt-0.5">Recent Transacted Sales (Same Street & Model)</h3>
          </div>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-200">
              {property.comparables.map((comp) => (
                <tr key={comp.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors font-medium">
                  <td className="py-3.5 px-3 font-semibold text-slate-900 dark:text-white">
                    Blk {comp.block} {comp.streetName}
                  </td>
                  <td className="py-3.5 px-3 text-slate-500 dark:text-slate-300">{comp.storeyRange}</td>
                  ...
                  <td className="py-3.5 px-3 font-mono text-slate-600 dark:text-slate-300">S${comp.pricePsf}</td>
                  <td className="py-3.5 px-3 text-slate-500 dark:text-slate-400">{comp.transactionDate}</td>
                  <td className="py-3.5 px-3 text-slate-500 dark:text-slate-400">{comp.distanceMeters}m away</td>
                </tr>
              ))}
            </tbody>
          </table>
